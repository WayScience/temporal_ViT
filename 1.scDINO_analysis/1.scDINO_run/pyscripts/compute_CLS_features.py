...
def extract_and_save_feature_pipeline(args):
    ...
    if not args.images_are_RGB:
        ...
        if args.normalize:
            ...
            mean_for_selected_channel, std_for_selected_channel = tuple(
                [norm_per_channel[0][mean] for mean in selected_channels]
            ), tuple([norm_per_channel[1][mean] for mean in selected_channels])
            print(
                "normalize with mean: ",
        ...
        class Multichannel_dataset(datasets.ImageFolder):
            def __getitem__(self, idx):
                ...
                if utils.check_nan(image_np):
                    ...
                    print(
                        "taking first image in dataset as replacement, so there are duplicates instead of nan values"
                    )
        ...
        print(os.path.join(args.dataset_dir))
        dataset_total = Multichannel_dataset(
            os.path.join(args.dataset_dir), transform=transform
        )
        print(dataset_total)
        print(dataset_total.samples[1])
        ...
        dataset_total = ReturnIndexDataset(
            os.path.join(args.dataset_dir), transform=transform
        )
    ...
    if args.use_weighted_sampler and args.class_weights and args.num_samples:
        ...
        weights_per_sample = [
            weights_per_class[dataset_total.samples[i][1]]
            for i in range(len(dataset_total.samples))
        ]
        weighted_sampler = torch.utils.data.sampler.WeightedRandomSampler(
            weights_per_sample, args.num_samples, replacement=False
        )
    ...
    print(
        f"Data loaded with {num_samples} images with a size of {local_batch.shape[2]}x{local_batch.shape[3]} with {num_channels} channels"
    )

    # ============ building network ... ============
    if "vit" in args.arch:
        num_in_chans_pretrained = utils.get_pretrained_weights_in_chans(
        ...
        model.eval()
        print(
            f"Model {args.arch} {args.patch_size}x{args.patch_size} with {num_in_chans_pretrained} in_chans built."
        )
    else:
    ...
    def embedding_seq(custom_embedding_map, selected_channels):
        ...
        embedding_seq = [
            map_dict[int(input_channel)] for input_channel in selected_channels
        ]
    ...
    if not args.use_mean_patch_embedding and not args.use_custom_embedding_map:
        if num_channels != num_in_chans_pretrained:
            print(
                f"Error: Number of channels in the dataset ({num_channels}) and pretrained weights ({num_in_chans_pretrained}) are different"
            )
            print(
                f"Use --use_mean_patch_embedding or --use_custom_embedding_map to adjust the number of channels"
            )
            raise ValueError(
                f"Number of channels in the dataset ({num_channels}) and pretrained weights ({num_in_chans_pretrained}) are different"
            )

    if not args.images_are_RGB:
        if args.use_mean_patch_embedding:
            average_conv2d_weights = torch.mean(
                model.patch_embed.proj.weight, 1, keepdim=True
            )
            conv2d_weights_per_chan = average_conv2d_weights.repeat(
                1, num_channels, 1, 1
            )
            model.patch_embed.proj.weight = nn.Parameter(conv2d_weights_per_chan)
        elif args.use_custom_embedding_map:
            embedding_seq = embedding_seq(
                args.custom_embedding_map, args.selected_channels
            )
    ...
    labels = [
        utils.fetch_foldername_of_img_location(
            dataset_total, i, args.folder_depth_for_labels
        )
        for i in index_all
    ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--batch_size_per_gpu", default=30, type=int, help="Per-GPU batch-size"
    )
    parser.add_argument(
        "--pretrained_weights",
        default="",
    ...
    parser.add_argument(
        "--images_are_RGB",
        help="If images are RGB, set this to True. If images are grayscale, set this to False.",
        default=False,
        type=utils.bool_flag,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--channel_dict",
        default=None,
        help="""name of the channels in format as dict channel_number, channel_name.""",
    )
    parser.add_argument(
        "--resize", default=False, help="""if images should be resized"""
    )
    parser.add_argument(
        "--resize_length",
        default=None,
        help="""quadratic resize length to resize images""",
    )
    parser.add_argument(
        "--norm_per_channel",
        default="[(x, x, x, x, x), (x, x, x, x, x)]",
        type=str,
        help="""2x tuple of mean and std per channel typically values between 0 and 1""",
    )
    parser.add_argument(
        "--norm_per_channel_file",
        default=None,
        help="""path to file with mean and std per channel in json format.""",
    )
    parser.add_argument(
        "--center_crop", default=None, help="""center crop factor to crop images"""
    )
    parser.add_argument(
        "--normalize",
        default="False",
        type=str,
        help="""normalize with mean and std per channel""",
    )
    parser.add_argument(
        "--patch_embedding_mapping",
        default=None,
        help="""change the patch embedding weights by inputting a string of the sequence of rearrangement of the model '[0,1,2]' or the string 'average_weights' or None""",
    )
    parser.add_argument(
        "--parse_params",
    ...
    parser.add_argument(
        "--class_weights", default=None, help="""list of weights for each class"""
    )
    parser.add_argument(
        "--num_samples",
        default=None,
        type=int,
        help="Number of images to run in total.",
    )
    parser.add_argument(
        "--read_model_arch_dynamically",
        default=None,
        type=str,
        help="Read model architecture from pretrained weights",
    )
    parser.add_argument(
        "--use_mean_patch_embedding",
        default=False,
        type=bool,
        help="Use mean patch embedding instead of first patch embedding",
    )
    parser.add_argument(
        "--use_custom_embedding_map",
        default=False,
        type=bool,
        help="Use custom embedding map",
    )
    parser.add_argument(
        "--custom_embedding_map", default=None, type=dict, help="Custom embedding map"
    )
    parser.add_argument(
        "--scDINO_full_pipeline",
        default=False,
        type=bool,
        help="Using scDINO full pipeline",
    )
    parser.add_argument(
        "--full_ViT_name",
        default="full_vit_name",
        type=str,
        help="name channel combi ViT",
    )
    parser.add_argument(
        "--train_datasetsplit_fraction",
        default=0.8,
        type=float,
        help="when using scDINO full pipeline",
    )
    parser.add_argument(
        "--test_datasetsplit_fraction",
        default=0.8,
        type=float,
        help="when using downstream analysis only",
    )
    parser.add_argument("--seed", default=42, type=int, help="Random seed.")
    parser.add_argument(
        "--folder_depth_for_labels",
        default=0,
        type=int,
        help="Folder depth for labels. 0 means that the labels are the folder names where the images are stored. 1 means one level above and so on. e.g  path/to/images/labelwhen3/labelwhen2/labelwhen1/labelwhen0/image.tiff",
    )
    ...
    if args.norm_per_channel_file:
        with open(args.norm_per_channel_file) as f:
            ...
            norm_per_channel = str(
                [
                    tuple(norm_per_channel_json["mean"]),
                    tuple(norm_per_channel_json["std"]),
                ]
            )
    ...
    if dist.get_rank() == 0:
        ...
        def get_channel_name_combi(channel_combi, channel_dict):
            ...
            for channel_number in iter(str(channel_combi)):
                name_of_channel_combi = "_".join(
                    [name_of_channel_combi, channel_dict[int(channel_number)]]
                )
        ...
        if args.scDINO_full_pipeline:
            ...
            cls_directory = os.path.join(
                args.output_dir, args.name_of_run, f"{ViT_name}_analyses/CLS_features"
            )
            ...
            cls_directory = os.path.join(
                args.output_dir, args.name_of_run, "CLS_features"
            )
            model_name = args.pretrained_weights.split("/")[-1].split(".")[0]
            epoch_path = os.path.join(
                cls_directory, f"channel{channel_names}_model_{model_name}"
            )
