...
class GaussianBlur_forGreyscaleMultiChan(object):
    """
    ...
    def __call__(self, tensor):
        ...
        if not do_it:
            ...
            blur = transforms.GaussianBlur(kernel_size=(5, 5), sigma=(0.1, 1.5))
            tensor = blur(tensor)
            return tensor
...
class Solarization_forGreyscaleMultiChan(object):
    """
    ...
    def __call__(self, tensor):
        if random.random() < self.p:
            tensor[tensor > self.threshold] = 1 - tensor[tensor > self.threshold]
            return tensor
        else:
            return tensor
...
class AdjustGamma_custom(object):
    ...
    def __call__(self, x):
        if random.random() < self.p:
            random_gamma = random.uniform(0.75, 3)
            for channel in range(x.shape[0]):
                x[channel] = gamma_adjust_wo_nan(x[channel], random_gamma, eps=self.eps)
            return x
        else:
            return x
...
def gamma_adjust_wo_nan(tensor, gamma, eps=1e-8, a=1):
    channel = tensor.numpy()
    gamma_adj_channel = ((np.abs(channel) + eps) ** gamma) * a * np.sign(channel)
    tensor_adj = torch.from_numpy(gamma_adj_channel).float()
    return tensor_adj
...
def init_distributed_mode(args):
    ...
    #     print("Use torch.distributed.launch")
    ...
    # launched with submitit on a slurm cluster
        ...
        print("Use submitit on a slurm cluster")
    ...
    # we manually add MASTER_ADDR and MASTER_PORT to env variables
        ...
        # define another random address to avoid address already in use error
